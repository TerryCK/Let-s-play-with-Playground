...
let count: Count? = Count(inbox: 10, notification: 20)

let transform : (Count) -> Int? = { (x: Count) -> Int? in
    return nil
}

//print( count.map(transform) )
//print(count.flatMap(transform) )
//
//let total = count.flatMap { $0.inbox + $0.notification }
//
//print(count?.name)
//
...
enum Language: TestProtocol{
    case English
    case Chineses

}
let en = Language.English
let ch = Language.Chineses
en == ch

en == Language.English

[en, ch] == [ Language.English, Language.Chineses]

let array = Array(0..<10).map { IndexPath(index: $0) }


let testProtocol : TestProtocol = en
...
if case (let result?, false) = (g, isTrue){
    print(result)
}
...
func error(x: Int) throws -> Int {
    if x < 0 {
        throw MyError.failure(error: "值太小了")
    } else if x == 0 {
        throw MyError.connection
    }
    
    return x
}

let result = try? error(x: 10)

let result2 = try? error(x: -1)
...
let decoded = try? JSONDecoder().decode(Array<Any>.self, from: json)
let data = decoded?.first as? [String: Any]
let id = data?["id"]
